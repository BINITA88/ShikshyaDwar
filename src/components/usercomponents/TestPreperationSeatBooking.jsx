...
const TestPreparationSeatBooking = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setError('You can booked seat only after full payment.');
    }
  }, [token]);
  ...
  const handleSeatBooking = async (seatNumber) => {
    try {
      ...
      toast.success(`Seat booked successfully for this Saturday!🎉`);
...
//       } catch (err) {
//         console.error('Error fetching seat status:', err);
//         setError('Failed to fetch seat status.');
...
//       {/* Monitor */}
//       <div className={`w-16 h-12 mx-auto mb-1 rounded-t-lg border-4 
//         ${seat.status === 'Booked' ? 'border-gray-300 bg-gray-200' : 'border-blue-300 bg-blue-50'}
//         transform group-hover:scale-105 transition-all duration-300`}>
//         <div className={`h-2 w-8 mx-auto mt-1 rounded
//           ${seat.status === 'Booked' ? 'bg-gray-400' : 'bg-blue-400'}`}></div>
//       </div>

//       {/* Stand */}
//       <div className={`w-4 h-4 mx-auto 
//         ${seat.status === 'Booked' ? 'bg-gray-300' : 'bg-blue-300'}`}></div>

//       {/* Desk */}
//       <div className={`w-20 h-6 mx-auto rounded-lg 
//         ${seat.status === 'Booked' ? 'bg-gray-200' : 'bg-blue-100'}
//         shadow-md transform group-hover:translate-y-1 transition-all duration-300`}>
//         <span className="text-xs font-medium flex items-center justify-center h-full text-blue-700">
//           {seat.seatNumber}
//         </span>
//       </div>
...
//       {/* Chair */}
//       <div className={`w-12 h-8 mx-auto mt-1 rounded-b-lg
//         ${seat.status === 'Booked' ? 'bg-gray-300' : 'bg-blue-200'}
//         transform group-hover:translate-y-1 transition-all duration-300`}>
//       </div>
//     </div>
//   );
...
//         {/* Error Message */}
//         {error && <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-r-lg">{error}</div>}

//         {/* Display Seats */}
//         <div>
//           <h2 className="text-2xl font-semibold text-gray-700 mb-4">Limited Seats Available</h2>
//           {seats.length > 0 ? (
//             <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
//               {seats.map((seat) => (
//                 <ComputerSeat
